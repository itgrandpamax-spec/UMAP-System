...
/**
 * Set the target position for the compass arrow
 * This should be the world position of Point B (destination marker)
 */
function setCompassTarget(targetPosition) {
  if (typeof THREE === 'undefined') {
  ...
  console.log('[AR_HUD] Compass target set to:', compassTarget);
  initializeCompassHUD();
}

/**
 * Update compass HUD - called each frame
 * Calculates angle from camera to target and draws directional arrow
 */
function updateCompassHUD() {
  if (!compassCanvas || !compassContext || !compassTarget) {
  ...
  // Get camera's right vector for compass coordinate system
  const cameraRight = new THREE.Vector3(1, 0, 0);
  cameraRight.applyQuaternion(camera.quaternion);
  
  // Calculate angle in screen space (horizontal plane)
  // Project to horizontal: ignore up component for compass bearing
  const horizontalForward = new THREE.Vector3(cameraForward.x, 0, cameraForward.z).normalize();
  const horizontalTarget = new THREE.Vector3(toTarget.x, 0, toTarget.z).normalize();
  ...
  // Draw compass HUD
  drawCompass(angleToTarget, distance);
}
...
function drawCompass(angle, distance) {
  ...
  compassContext.fillStyle = 'rgba(0, 0, 0, 0.6)';
  compassContext.beginPath();
  compassContext.arc(centerX, centerY, radius, 0, Math.PI * 2);
  compassContext.fill();
  
  // Draw border circle
  compassContext.strokeStyle = 'rgba(255, 255, 255, 0.8)';
  compassContext.lineWidth = 2;
  compassContext.beginPath();
  compassContext.arc(centerX, centerY, radius, 0, Math.PI * 2);
  compassContext.stroke();
  ...
  compassContext.fillStyle = 'rgba(255, 255, 255, 0.5)';
  compassContext.font = 'bold 10px sans-serif';
  compassContext.textAlign = 'center';
  compassContext.textBaseline = 'middle';
  
  // North marker (top)
  compassContext.fillText('N', centerX, centerY - radius + 8);
  ...
  compassContext.moveTo(arrowX, arrowY);
  compassContext.lineTo(
    arrowX - headSize * Math.sin(headAngle - Math.PI / 6),
    arrowY + headSize * Math.cos(headAngle - Math.PI / 6)
  );
  compassContext.lineTo(
    arrowX - headSize * Math.sin(headAngle + Math.PI / 6),
    arrowY + headSize * Math.cos(headAngle + Math.PI / 6)
  ...
  // Draw distance text
  let distanceText = distance.toFixed(1) + 'm';
  compassContext.fillStyle = 'rgba(255, 255, 255, 0.8)';
  compassContext.font = 'bold 12px sans-serif';
  compassContext.textAlign = 'center';
  compassContext.textBaseline = 'top';
  compassContext.fillText(distanceText, centerX, centerY + radius - 12);
}

/**
 * Hide the compass HUD
 */
function hideCompassHUD() {
  if (compassCanvas) {
    compassCanvas.style.display = 'none';
...
/**
 * Clear the compass target and hide HUD
 */
function clearCompass() {
  compassTarget = null;
  hideCompassHUD();
...
console.log('[AR_HUD] HUD module loaded successfully');
