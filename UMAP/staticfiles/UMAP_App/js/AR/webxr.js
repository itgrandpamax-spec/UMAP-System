...
async function startARSession() {
  ...
  try {
    const sessionInit = { requiredFeatures: ['hit-test','local-floor'], optionalFeatures: ['dom-overlay'], domOverlay: { root: document.getElementById('overlay-controls') || document.body } };
  ...
  try {
    ...
    } catch(e) { console.warn('Failed to unbind markers for AR', e); }
  ...
  window._AR_WEBXR.xrSession = xrSession;
    ...
    // restore markers bind state after leaving XR
    try {
...
function onXRFrame(time, xrFrame) {
  const session = xrFrame.session; session.requestAnimationFrame(onXRFrame);
  const pose = xrFrame.getViewerPose(xrRefSpace);
  if (pose) {
    const view = pose.views[0]; const viewport = session.renderState.baseLayer.getViewport(view);
  const arRenderer = getARRenderer(); if (arRenderer) arRenderer.setSize(viewport.width, viewport.height);
  ...
  if (arRenderer2 && arScene2 && arCamera2) arRenderer2.render(arScene2, arCamera2);
