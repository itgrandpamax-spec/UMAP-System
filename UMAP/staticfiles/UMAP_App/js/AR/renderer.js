...
if (!window.updateDebug) window.updateDebug = function(text) { try { const s = document.getElementById('dbg-status'); if (s) s.textContent = text; else console.log('DBG:', text); } catch (e) { console.log('DBG:', text); } };
...
async function initThree() {
  try {
    const mod = await import('three'); THREE_module = mod; window.THREE = THREE_module;
    const loaderMod = await import('https://unpkg.com/three@0.160.0/examples/jsm/loaders/GLTFLoader.js'); GLTFLoaderClass = loaderMod.GLTFLoader;

    // create or reuse a canvas for the preview
    let canvas = document.getElementById('overlay');
    if (!canvas) {
      canvas = document.createElement('canvas');
      canvas.id = 'overlay';
      canvas.style.position = 'fixed'; canvas.style.left = '0'; canvas.style.top = '0'; canvas.style.width = '100vw'; canvas.style.height = '100vh'; canvas.style.zIndex = '9999';
      document.body.appendChild(canvas);
    }

    threeRenderer = new THREE_module.WebGLRenderer({ canvas: canvas, alpha: true });
    threeRenderer.setPixelRatio(window.devicePixelRatio || 1);
    threeRenderer.setSize(window.innerWidth, window.innerHeight);
    threeRenderer.setClearColor(0x000000, 0);

    scene = new THREE_module.Scene();
    const modelRoot = new THREE_module.Object3D(); modelRoot.name = 'modelRoot'; scene.add(modelRoot); window._AR_RENDERER.modelRoot = modelRoot;

    const hemi = new THREE_module.HemisphereLight(0xffffff, 0x444444, 0.6); scene.add(hemi);
    const dir = new THREE_module.DirectionalLight(0xffffff, 0.8); dir.position.set(5,10,7.5); scene.add(dir);

    camera = new THREE_module.PerspectiveCamera(75, window.innerWidth/window.innerHeight, 0.1, 1000);
    ...
    try {
      ...
      controls.enableDamping = true; controls.dampingFactor = 0.07; controls.screenSpacePanning = false; controls.minDistance = 0.5; controls.maxDistance = 200;
      controls.target.set(0,0,0); controls.update(); window._AR_RENDERER.controls = controls;
    } catch(e) { console.warn('OrbitControls not available', e); }

    window._AR_RENDERER.renderer = threeRenderer; window._AR_RENDERER.scene = scene; window._AR_RENDERER.camera = camera;
    updateDebug('Three.js initialized (ESM)');
  } catch(e) {
    console.error('Failed to initialize Three (ESM):', e);
    updateDebug('Failed to initialize Three (ESM): ' + (e && e.message ? e.message : e));
  }
}
initThree();

const CANONICAL_GLTF = '/UMAP_App/glb/Umak_3d.glb';
