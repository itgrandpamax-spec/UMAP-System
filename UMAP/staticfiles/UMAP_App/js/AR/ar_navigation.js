...
window.updateDebug = function(text) { const s = document.getElementById('dbg-status'); if (s) s.textContent = text; else console.log('DBG:', text); };
window.rooms = [];
...
async function loadRoomsFromServer() {
  ...
  window.rooms = (roomsArray || []).map(r => ({ id: String(r.id), number: String(r.number || r.id), name: (r.name && r.name.trim()) || (r.number ? ('Room ' + r.number) : ('Room ' + r.id)), x: Number(r.x) || 0, y: Number(r.y) || 0, z: Number(r.z) || 0 }));
  if (window._AR_UI && window._AR_UI.populateRoomSelects) window._AR_UI.populateRoomSelects(window.rooms.map(r => ({ id: r.id, name: r.name })));
    // DISABLED: Don't render all markers on init - wait for explicit AR navigation via UserAR.js modal
    // if (window._AR_MARKERS && window._AR_MARKERS.highlightOtherRooms) window._AR_MARKERS.highlightOtherRooms(window.rooms, window.userRoomId);
  try { window._AR_EVENTS.emit('rooms:updated', window.rooms); } catch(e) {}
    ...
    // No fallback rooms — wait for server data or manual update
    console.error('[AR] Room load error:', err);
  }
}
...
window.localizeAR = function(selectedRoomId) {
  ...
  // DISABLED: Don't render all markers in setTargetRoom - use explicit AR navigation flow via UserAR.js
...
window.moveCameraToRoom = function(room) {
  try {
    ...
    if (rendererObj && rendererObj.modelRoot) {
      // transform by modelRoot's world matrix
      worldPos.applyMatrix4(rendererObj.modelRoot.matrixWorld || rendererObj.modelRoot.matrix);
    }
    // move camera/controls
    if (controls) {
      controls.target.copy(worldPos);
      // place camera a bit back from the target (maintaining height from room.z)
      camera.position.set(worldPos.x + 0, worldPos.y + 5, worldPos.z + 8);
      controls.update();
    } else if (camera) {
      camera.position.set(worldPos.x + 0, worldPos.y + 5, worldPos.z + 8);
...
window.setTargetRoom = function(roomId) {
  ...
  try {
      ...
      if (localizeFn) {
        localizeFn(room).then(xrLoc => {
          try {
            ...
            window.userPos = { x: room.x, y: room.y, z: room.z };
            window.userRoomId = room.id;
            // DISABLED: Don't render all markers on XR localization - use explicit AR navigation flow
            // if (window._AR_MARKERS && window._AR_MARKERS.highlightOtherRooms) window._AR_MARKERS.highlightOtherRooms(window.rooms, window.userRoomId);
    ...
    if (window.moveCameraToRoom) {
      ...
      window.userPos = { x: room.x, y: room.y, z: room.z };
      window.userRoomId = room.id;
      // DISABLED: Don't render all markers on preview mode - use explicit AR navigation flow via UserAR.js
      // if (window._AR_MARKERS && window._AR_MARKERS.highlightOtherRooms) window._AR_MARKERS.highlightOtherRooms(window.rooms, window.userRoomId);
